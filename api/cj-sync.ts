...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const products = linkBlocks.slice(0, 12).map((block, i) => { const advertiserName = xmlValue(block, "advertiser-name") || "CJ advertiser"; const title = xmlValue(block, "link-name") || advertiserName || "CJ partner offer"; const isCar = /carmel|limo|taxi|rental|car/i.test(`${advertiserName} ${title}`); return { id: xmlValue(block, "link-id") || `cj-link-${i}`, title, description: xmlValue(block, "description") || "Promotional offer from a CJ advertiser.", price: "View offer", currency: "", advertiserName, clickUrl: xmlValue(block, "click-url") || xmlValue(block, "clickUrl"), imageUrl: xmlValue(block, "creative-image-url") || xmlValue(block, "image-url") || (isCar ? carImages[i % carImages.length] : images[i % images.length]), syncedAt: new Date().toISOString() }; });
