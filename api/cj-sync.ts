...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!ENV.databaseUrl) return res.status(503).json({ ok: false, message: "CJ links were fetched, but no Vercel/Supabase PostgreSQL URL is configured, so products could not be saved.", products });
    pool = new Pool({ connectionString: ENV.databaseUrl, ssl: { rejectUnauthorized: false }, max: 2 });
