...
const channels = [
  { name: "Instagram", icon: Instagram, status: "Ready for connection", color: "text-pink-600", bg: "bg-pink-50" },
  { name: "Facebook", icon: Users, status: "Ready for connection", color: "text-blue-600", bg: "bg-blue-50" },
  { name: "YouTube", icon: Youtube, status: "Ready for connection", color: "text-red-600", bg: "bg-red-50" },
];
  ...
  return (
    <div className="min-h-screen bg-[#f8f6f1] text-stone-950">
      ...
      <main id="top">
        ...
        <section id="content" className="scroll-mt-24 bg-stone-950 px-5 py-16 text-white lg:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-amber-300">Field notes</p><h2 className="mt-3 text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">Content people can actually use.</h2><p className="mt-5 max-w-md leading-7 text-stone-400">One approved product brief becomes a channel-specific package: short-video script, caption, title, CTA, disclosure, and trackable destination.</p><Button className="mt-8 bg-amber-200 text-stone-950 hover:bg-amber-100" onClick={() => setActiveTab("studio")}>Open content studio<ArrowUpRight className="ml-2 h-4 w-4" /></Button></div><div className="grid gap-4 sm:grid-cols-3">{channels.map(({ name, icon: Icon, status, color, bg }) => <div key={name} className="rounded-3xl border border-white/10 bg-white/[0.06] p-5"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${bg} ${color}`}><Icon className="h-5 w-5" /></div><h3 className="mt-7 font-semibold">{name}</h3><p className="mt-2 text-sm leading-6 text-stone-500">{status}</p><div className="mt-7 flex items-center gap-2 text-xs font-medium text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" />Official API-ready</div></div>)}</div></div></section>
      ...
      <section className="border-t border-stone-200 bg-white/60 px-5 py-8 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 text-sm text-stone-500 sm:flex-row"><span>© 2026 Arc &amp; Co. A working commerce foundation.</span><span className="flex items-center gap-2"><LockKeyhole className="h-4 w-4" />Privacy-aware analytics · Affiliate disclosure</span></div></section>

      {activeTab !== "storefront" && <div className="fixed inset-0 z-40 bg-stone-950/35 backdrop-blur-sm" onClick={() => setActiveTab("storefront")}><aside className="absolute right-0 top-0 h-full w-full max-w-2xl overflow-y-auto bg-[#f8f6f1] p-6 shadow-2xl sm:p-10" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-stone-500">Control room</p><h2 className="mt-2 text-3xl font-semibold">AutoCommerce OS</h2></div><Button variant="ghost" size="icon" onClick={() => setActiveTab("storefront")}><X /></Button></div><Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="mt-8"><TabsList className="grid w-full grid-cols-3 bg-stone-200/60"><TabsTrigger value="studio">Content studio</TabsTrigger><TabsTrigger value="analytics">Analytics</TabsTrigger><TabsTrigger value="settings">Guardrails</TabsTrigger></TabsList><TabsContent value="studio" className="mt-6 space-y-4"><Card><CardHeader><div className="flex items-center justify-between"><div><CardTitle>Next package: Halo Desk Light</CardTitle><p className="mt-1 text-sm text-stone-500">3 channel variants · disclosure included</p></div><Badge className="bg-amber-100 text-amber-800">Draft</Badge></div></CardHeader><CardContent className="space-y-3"><div className="rounded-2xl bg-stone-950 p-5 text-white"><div className="flex items-center gap-2 text-xs uppercase tracking-widest text-amber-200"><Play className="h-3 w-3" />Instagram Reel script</div><p className="mt-4 text-lg leading-7">“A softer workspace starts with one small change. Here’s what this compact desk light does well, where it fits, and the offer we found today.”</p></div><div className="grid gap-3 sm:grid-cols-3">{["Instagram", "Facebook", "YouTube"].map((channel) => <Button key={channel} variant="outline" className="justify-between border-stone-300 bg-white/40" onClick={() => toast(`${channel} package queued for review`)}>{channel}<ChevronRight className="h-4 w-4" /></Button>)}</div><p className="flex items-center gap-2 text-xs text-stone-500"><Check className="h-4 w-4 text-emerald-600" />Claim scan passed · tracking link attached · manual approval enabled</p></CardContent></Card></TabsContent><TabsContent value="analytics" className="mt-6 space-y-4"><div className="grid grid-cols-2 gap-3"><Metric label="Outbound clicks" value="1,284" change="+18.4%" icon={Link2} /><Metric label="Offers viewed" value="3,906" change="+11.2%" icon={TrendingUp} /><Metric label="Content saves" value="428" change="+9.7%" icon={BarChart3} /><Metric label="Attribution" value="76%" change="Consent-aware" icon={Gauge} /></div><Card><CardHeader><CardTitle>Channel signal</CardTitle></CardHeader><CardContent className="space-y-4">{[["Instagram", "68%", "bg-pink-500"], ["Facebook", "44%", "bg-blue-500"], ["YouTube", "52%", "bg-red-500"]].map(([name, value, color]) => <div key={name}><div className="mb-2 flex justify-between text-sm"><span>{name}</span><span className="font-semibold">{value}</span></div><div className="h-2 rounded-full bg-stone-100"><div className={`h-2 rounded-full ${color}`} style={{ width: value }} /></div></div>)}</CardContent></Card></TabsContent><TabsContent value="settings" className="mt-6 space-y-4"><Card><CardHeader><CardTitle>Automation guardrails</CardTitle></CardHeader><CardContent className="space-y-4">{["Reject unsupported health, income, or performance claims", "Pause offers when destination or stock checks fail", "Require a disclosure on partner content", "Use official platform integrations only", "Keep a safe manual fallback when publishing is blocked"].map((rule) => <div key={rule} className="flex items-start gap-3 text-sm"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><Check className="h-3 w-3" /></span><span>{rule}</span></div>)}</CardContent></Card><Card className="border-orange-200 bg-orange-50/60"><CardContent className="flex gap-3 p-5 text-sm leading-6 text-orange-900"><CircleHelp className="mt-0.5 h-5 w-5 shrink-0" /><span>Social accounts are shown as API-ready, not connected. The production setup will require your own approved platform credentials and business account permissions.</span></CardContent></Card><Card><CardHeader><CardTitle>Niche growth loop</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3">{[["Teach", "Practical how-to posts"], ["Compare", "Clear product trade-offs"], ["Explain", "Why this made the edit"]].map(([label, detail]) => <div key={label} className="rounded-2xl bg-stone-100 p-4"><div className="text-xs font-bold uppercase tracking-widest text-stone-500">{label}</div><div className="mt-2 text-sm font-semibold">{detail}</div><div className="mt-3 text-xs text-emerald-700">Scheduled format</div></div>)}</CardContent></Card></TabsContent></Tabs></aside></div>}
    </div>
  );
}
