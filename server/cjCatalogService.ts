...
export async function getSyncedAffiliateProducts(): Promise<CjProduct[]> {
  ...
  try {
    ...
    const awinResult = await db.execute(sql`SELECT atl.external_link_id AS "externalLinkId", atl.token AS "trackingToken", 'awin' AS network, ap.title, ap.description, ap.price, ap.currency, ap.advertiser_name AS "advertiserName", atl.destination_url AS "clickUrl", atl.image_url AS "imageUrl", atl.last_seen_at AS "syncedAt" FROM awin_tracked_links atl INNER JOIN awin_products ap ON ap.id = atl.awin_product_id WHERE atl.link_status = 'active' AND ap.status = 'active' ORDER BY atl.last_seen_at DESC LIMIT 500`);
    const awinRows = (awinResult as unknown as { rows?: Array<Record<string, unknown>> }).rows || (awinResult as unknown as Array<Record<string, unknown>>);
